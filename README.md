...
`tools/list` at `https://gateway.pipeworx.io/boj/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/boj_policy_rate \
  -H 'Content-Type: application/json' \
  -d '{"months":12}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/boj_policy_rate`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```
ask_pipeworx({ question: "your question about Boj data" })
```
